...
| **[51 — Charts & Dashboards System Overview](./51-charts-system-overview.md)** | **Tổng quan toàn bộ hệ thống chart Hdos.** Sơ đồ kiến trúc, decision matrix Path A vs Path B, endpoint catalog (đã/chưa wire), file structure, deploy status (2026-06-08). Đọc TRƯỚC khi đi sâu vào doc 48/49/50 |
| [52 — Embed Lakehouse Chart vào DynamicForm Screen](./52-embed-lakehouse-chart-in-dynamicform.md) | Bridge Path B chart (`/lakehouse/charts/{code}`) vào DynamicForm screen designer. Mức 1 (Provider/Operation catalog) đã DONE trên server. Mức 2 (WidgetCatalog `embed_sdui_page` + FE renderer TypeScript) đề xuất code mẫu. Companion với doc 41/48/50/51 |
| **[53 — Data Contract Engine: "Phễu" Universal](./53-chart-funnel-architecture.md)** | **Hợp nhất 3 pattern rời rạc** (SourceProfile + SduiPageConfig + ILakehouseChartConfig) thành 1 abstraction 3 tầng Contract / Source / Consumer + `DataContractGateway`. Schema = C# record type-safe. Pilot FinanceDaily với 3 source (view/sql/demo) + 2 consumer (chart/csv). Migration progressive P1→P7, endpoint cũ giữ nguyên. Update Path A/B sang DataContract recipe |
| **[54 — Walkthrough: smoke test + new contract BE+FE](./54-data-contract-walkthrough.md)** | **Hands-on tutorial** companion với doc 53. Phần 1: smoke test pilot FinanceDaily (~10 phút) — bật flag, hit endpoint, verify JSON. Phần 2: build contract MỚI `patient.daily.new` end-to-end (~30-45 phút) — schema record, 2 sources (demo + sql), chart consumer, DI register, Next.js FE page. Plus troubleshooting + checklist cheat sheet |
| **[55 — Workflow: nghiệp vụ → working chart](./55-workflow-business-to-chart.md)** | **Decision tree + process** companion với doc 54. 8 bước từ business need → user thấy chart: mockup → fields list → quyết định service owner → quyết định source kind → code top-down → curl test → FE page → iterate. Decision tree chọn service + source kind. Anti-patterns. Cheat sheet bảng quyết định nhanh |
| **[56 — Data Contract files BE đã thêm + ý nghĩa](./56-data-contract-files-explained.md)** | **Reference cho dev mới + FE team.** Liệt kê 3 nhóm file BE: A (framework BuildingBlocks 10 file), B (per-chart 3-7 file/contract — vd Finance, Patient), C (wiring 3 file). Bảng meaning từng file. Cheat sheet URL endpoints + JSON shape FE cần render |
| **[57 — SDUI BE↔FE Contract Handbook](./57-sdui-be-fe-contract-handbook.md)** | **Chuẩn hợp đồng để FE triển khai.** Vocabulary 16 widget, JSON envelope (ApiResponse + SduiPage), BE responsibilities (Consumer + DI + error codes), FE responsibilities (REGISTRY/MANIFEST + generic page /dashboards/[code] + loading/error/empty), workflow Case A (BE-only) vs Case B (FE-first), versioning, naming chuẩn (color hex, dates, money), code review checklist, end-to-end example finance.daily.row |
| **[58 — Lakehouse DataContract ↔ DynamicForm Integration](./58-lakehouse-dynamicform-integration.md)** | **Bridge 2-tier registration** giữa Lakehouse DataContract engine (Tier 1: DI code) và DynamicForm Provider/Operation catalog (Tier 2: DB seed). Phase 1 DONE: seed Provider `lakehouse` + 2 Operations (prefill, chart) qua `HasData()`, thêm `?mode=single` cho `FormPrefillResult` để expression `{{sources.ns.field}}` bind 1 record phẳng. Step-by-step admin tạo screen bind contract end-to-end. Roadmap Phase 2-4 (schema discovery, auto-gen form, auto-sync). Companion doc 41/52/53/54 |
| **[61 — DataSource `defaultParams`: lưu giá trị mặc định cho `{param}` placeholder](./61-datasource-default-params.md)** | Thêm field `defaultParams: Dictionary<string,string>?` vào DataSource VO (DynamicFormService) cho phép admin gán default value cho `{contractCode}` và các placeholder khác trong `resourcePath`. FE merge với URL params (URL wins). Không cần migration (jsonb auto). 4 files sửa: VO, SetCommand, GetQuery, DataSourceDto. Backward-compat: row cũ → `null` → FE fallback về URL params. Companion doc 41/58/60 |
| **[64 — Apache Superset Phase 1: Standalone](./64-superset-phase1-standalone.md)** | **BI module độc lập** sau nginx subpath `/superset/`. 3 services Docker (postgres-superset, superset-init, superset), Werkzeug ProxyFix cho subpath. Login admin/admin. Roadmap 6 phase. CI/CD: image build trong matrix, push GHCR, server overlay |
| **[65 — Apache Superset Phase 2: SSO với AuthService](./65-superset-phase2-sso.md)** | Auto-login Superset bằng JWT Hdos. Custom Security Manager Python (HS256 share `Jwt__Secret`), AuthService endpoint `/auth/superset/sso` set cookie scope `/superset/`. Role mapping: claim `permission` → Superset Admin/Alpha/Gamma. Tech debt RS256+JWKS. Companion doc 06/64 |
| **[66 — Apache Superset Phase 4: Embedded Dashboard (FE Guide)](./66-superset-phase4-fe-embedded-guide.md)** | Nhúng dashboard vào FE Hdos qua iframe + guest token (TTL 5 phút). BE endpoint `POST /auth/superset/guest-token` proxy gọi Superset Admin API. FE dùng `@superset-ui/embedded-sdk`. Hướng dẫn admin bật embed, RLS filter động, troubleshooting CORS/Talisman. Companion doc 64/65 |
